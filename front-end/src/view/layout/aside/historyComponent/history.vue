  ...
  <div class="router-history">
    ...
    >
      <el-tab-pane
        ...
        :tab="item"
      >
      <!-- 修改了自定义插槽label用来实现带图标的tab页面-->
      <!-- 2022.10.27 wyy
          暂时取消 后面如果需要自定义 再打开 
      -->
      <!-- <template #label>
          <el-button class="transparent-button" :icon="item.meta.icon">{{item.params.title || item.meta.title}}</el-button>
      </template> -->
      </el-tab-pane>
    </el-tabs>
...
watch(() => historys.value, () => {
  // 不显示二级菜单
  // historys.value.forEach((item, index) => {
  //   if(item.meta.hidden) {
  //     if(item.meta.showInHistory === true) {
